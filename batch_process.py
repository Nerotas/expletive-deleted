...
from censor_profanity import ProfanityCensor, transcript_cache_is_compatible
...
def output_path(input_file: Path, transcoded_dir: Path) -> Path:
    extension = ".mp3" if input_file.suffix.lower() in AUDIO_EXTENSIONS else ".mkv"
    return transcoded_dir / f"{input_file.stem}-censored{extension}"
...
def process_file(
    ...
    destination = output_path(input_file, paths.transcoded)
...
def main() -> int:
    ...
    print(f"Transcoded folder: {paths.transcoded}")
    ...
    if all_discovered:
        ...
        for word in sorted(all_discovered):
            print(f"  {word}")
        print("Add to profanity_censor_words.txt to censor, or profanity_exclusions.txt to ignore.")
    else:
