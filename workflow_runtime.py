...
class RuntimePaths:
    ...
    def create(self) -> None:
        for path in (self.ready, self.transcoded, self.processed, self.transcripts):
...
def get_runtime_paths(root: Path | None = None) -> RuntimePaths:
    ...
    project_root = Path(root or os.environ.get("CENSOR_PROJECT_ROOT", PROJECT_ROOT)).resolve()
    return RuntimePaths(
        root=project_root,
        ready=project_root / "ready",
        transcoded=project_root / "transcoded",
...
def get_profanity_exclusions_file(root: Path | None = None) -> Path:
    ...
    if not configured:
        parser = read_project_config(project_root)
        configured = parser.get("Profanity", "ExclusionsFile", fallback="profanity_exclusions.txt").strip()
...
def get_profanity_censor_words_file(root: Path | None = None) -> Path:
    ...
    if not configured:
        parser = read_project_config(project_root)
        configured = parser.get("Profanity", "CensorWordsFile", fallback="profanity_censor_words.txt").strip()
