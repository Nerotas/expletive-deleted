...
type DictionaryInfo = { words_path: string; words_count: number; words: string[]; exclusions_path: string; exclusions_count: number; exclusions: string[]; changed?: boolean }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {page === 'queue' ? <section className="page queue-page">
        ...
        <div className="table-frame"><table><thead><tr><th>File</th><th>Status</th><th>Progress</th><th>Details</th></tr></thead><tbody>{mergedRows.map(({ item, job }) => { const status = job?.status ?? item.status; const percent = job?.progress_percent; const event = job ? jobEvents[job.id] : undefined; const detail = event?.fps ? `${Math.round(event.fps)} FPS${event.eta_seconds != null ? ` · ${formatEta(event.eta_seconds)} left` : ''}` : event?.eta_seconds != null ? `${formatEta(event.eta_seconds)} left` : job?.error?.detail ?? (status === 'transcribed' ? 'Report available' : ['completed', 'finished'].includes(status) ? 'Output verified' : activeJob?.id === job?.id ? 'Processing locally' : 'Waiting'); return <tr key={item.source}><td><div className="file-cell"><span className="file-icon">{fileName(item.source).split('.').pop()?.toUpperCase()}</span><div><strong>{fileName(item.source)}</strong><small>{item.source}</small></div></div></td><td><StatusBadge status={status} /></td><td>{percent != null ? <div className="progress-wrap"><div className="progress-track"><span style={{ width: `${percent}%` }} /></div><span>{Math.round(percent)}%</span></div> : <span className="muted">—</span>}</td><td className="details-cell"><span>{detail}</span>{item.transcript && <button onClick={() => void openReview(item.source)}>Review words</button>}{job?.status === 'failed' && job.error?.retryable && <button onClick={() => retryJob(job)}>Retry</button>}</td></tr> })}{!loading && mergedRows.length === 0 && <tr><td colSpan={4}><div className="empty-state"><FolderOpen size={28} /><strong>No media in Ready</strong><span>Add a supported audio or video file to the configured input folder.</span></div></td></tr>}</tbody></table>{loading && <div className="loading-row"><LoaderCircle className="spin" size={20} />Reading local library</div>}</div>
      ...
      </section> : settings ? <SettingsPage settings={settings} setSettings={setSettings} chooseDirectory={chooseDirectory} save={saveSettings} busy={busy} capabilities={capabilities} dictionary={dictionary} updateDictionary={updateDictionary} /> : <div className="loading-row"><LoaderCircle className="spin" size={20} />Loading settings</div>}
    </main>
...
function SettingsPage({ settings, setSettings, chooseDirectory, save, busy, capabilities, dictionary, updateDictionary }: { settings: Settings; setSettings: (value: Settings) => void; chooseDirectory: (key: keyof Settings['directories']) => void; save: () => void; busy: boolean; capabilities: Capabilities | null; dictionary: DictionaryInfo | null; updateDictionary: (target: 'censor' | 'exclude', word: string, action?: 'add' | 'remove') => Promise<void> }) {
  ...
  return <section className="page settings-page"><PageHeading title="Settings" subtitle="Persistent preferences for local processing"><button className="button secondary" onClick={() => window.location.reload()}><RotateCcw size={17} />Discard</button><button className="button primary" disabled={busy} onClick={save}><Save size={17} />Save changes</button></PageHeading><div className="settings-layout">
    ...
    <SettingsSection title="Profanity dictionary" description="Edit the local policy used for future jobs"><DictionaryEditor dictionary={dictionary} busy={busy} updateDictionary={updateDictionary} /></SettingsSection>
