...
Per the audited plan in [docs/BUNDLED_RUNTIME_PACKAGING_PLAN.md](docs/BUNDLED_RUNTIME_PACKAGING_PLAN.md), the packaged Windows installer bundles only a private CPython runtime, its standard library, and the pip bootstrap required for consent-driven setup. Required processing packages, FFmpeg/FFprobe, yt-dlp, Deno, and the Whisper model are never bundled. The app detects compatible existing components or presents an exact, user-approved plan before obtaining them. This is a deliberate, explicit product decision: do not add another bundled third-party component without updating the plan and audit scripts, explicit product approval, and a licensing review.

A source checkout (the normal `npm run dev` developer workflow) uses the repository `.venv`; the packaged app uses its audited private Python runtime. Both workflows use the managed, prompted setup path for processing packages, FFmpeg, yt-dlp, Deno, and the model. Electron itself requires and distributes its framework-owned root `ffmpeg.dll` for Chromium codec support. That DLL is allowed as part of Electron, is not the external processing runtime, and must never satisfy FFmpeg/FFprobe readiness checks. The packaged installer must contain no external FFmpeg/FFprobe build, yt-dlp, Deno, Whisper model payload, or processing Python package; the package audit enforces this against the Python-only runtime manifest.

The desktop application must make this setup process as easy as practical. The managed-download flow below is the mechanism for every processing component not included with private Python.
...
- Do not silently download, install, bundle, redistribute, or modify third-party components. Every managed setup action requires an inspectable plan and explicit approval.
